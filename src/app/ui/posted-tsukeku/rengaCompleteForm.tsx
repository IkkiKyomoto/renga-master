  ...
  return (
    <form onSubmit={handleSubmit}>
        ...
        {tsukekus.map((tsukeku, i) => {
          return (
            <div key={i} onClick={handleClick} id={`${tsukeku.id}`} className={selected === `${tsukeku.id}` ? styles.card_selected  : styles.card_right_dashed}>
                <TsukekuCard
                  yonku={tsukeku.yonku}
                  goku={tsukeku.goku}
        ...
        <div>
      {tsukekus.length > 0 ? <button type="submit" className="mt-10 mb-10 bg-teal-500 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">投稿する</button>: <p>付句がまだ届いていません</p>}
